...
#if defined(DEBUG)
#define WLLog(level, ...) \
    if (level <= logLevel) { IOLog("WiFiDriver: %s %s: %u: ", __FILE__ ,__FUNCTION__, __LINE__); IOLog(__VA_ARGS__); IOLog("\n"); }
#else
#define WLLog(level, ...) \
    if (level <= logLevel) { IOLog("WiFiDriver: %s: %u: ",__FUNCTION__, __LINE__); IOLog(__VA_ARGS__); IOLog("\n"); }
#endif
